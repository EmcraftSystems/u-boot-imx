...
#define CONFIG_FLEXSPI_IOSIZE		0x00000300
#define CONFIG_FLEXSPI_FLASHBASE	0x60000000
#define CONFIG_FLEXSPI_FLASHSIZE	0x00800000
#define CONFIG_FLEXSPI_RXFIFO_SIZE	128
#define CONFIG_FLEXSPI_TXFIFO_SIZE	128
...
static void flexspi_set_lut(struct fsl_flexspi_priv *priv)
{
	...
	int rxfifo = CONFIG_FLEXSPI_RXFIFO_SIZE;
...
static int fsl_flexspi_get_seqid(struct fsl_flexspi_priv *flex, u8 cmd)
{
		...
		const void *dout, void *din, unsigned long flags)
{
	...
	if (dout) {
		if (flags & SPI_XFER_BEGIN) {
			cmd = *(u8 *)dout;
			memcpy(&txbuf, dout, 4);
			priv->cur_seqid = fsl_flexspi_get_seqid(priv, cmd);
		...
		unsigned int max_hz, unsigned int mode)
{
	...
	flexspi->priv.iobase = map_sysmem(CONFIG_FLEXSPI_IOBASE, CONFIG_FLEXSPI_IOSIZE);
	flexspi->priv.flashbase = map_sysmem(CONFIG_FLEXSPI_FLASHBASE, CONFIG_FLEXSPI_FLASHSIZE);
	flexspi->slave.max_write_size = CONFIG_FLEXSPI_TXFIFO_SIZE;

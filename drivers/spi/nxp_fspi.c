...
__weak ulong get_fspi_clk(int dev_id)
{
	return 0;
}

static ulong nxp_fspi_get_speed(struct udevice *bus)
{
#if CONFIG_IS_ENABLED(CLK)
	struct nxp_fspi *f = dev_get_priv(bus);

	return clk_get_rate(&f->clk);
#else
	return get_fspi_clk(dev_seq(bus));
#endif
}
...
static void nxp_fspi_select_mem(struct nxp_fspi *f, int chip_select)
{
	...
	/* Return if the selected chip-select is already active. */
	if (f->selected == chip_select)
		return;
